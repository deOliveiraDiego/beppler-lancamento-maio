...
### DATETIME
A data injetada pelo n8n (`$now`) pode ser hoje, 15/09. **Ignore essa data para abrir ou fechar o carrinho.** A única fonte é o `status` de `get_links`. Se vier `aberto`, venda — as tools de teste estão com relógio travado em 21/09 10h10.
